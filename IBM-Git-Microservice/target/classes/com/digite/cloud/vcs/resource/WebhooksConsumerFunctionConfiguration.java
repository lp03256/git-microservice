...
public class WebhooksConsumerFunctionConfiguration {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(WebhooksConsumerFunctionConfiguration.class);
    ...
    public RouterFunction<ServerResponse> webhooksConsumer() {
        ...
        route().POST("/webhooks/v1/{webhookId}", accept(MediaType.APPLICATION_JSON), serverRequest -> repository.findByWebhookId(serverRequest.pathVariable(webhookIdPathVar)).switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND, "webhook does not exist"))).doOnNext(webhookConfig -> {
            if (!serverRequest.headers().header(webhookConfig.getTokenHeaderName()).isEmpty() && !serverRequest.headers().firstHeader(webhookConfig.getTokenHeaderName()).equals(webhookConfig.getTokenHeaderValue())) throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "webhook token header is invalid");
        }).flatMap(webhookConfig -> serverRequest.bodyToMono(new ParameterizedTypeReference<Map<String, Object>>() {
        }).switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND, "webhook does not exist"))).flatMap(payload -> {
        ...
        })).flatMap(sent -> ServerResponse.ok().build()).log(this.getClass().getName(), Level.FINE).doOnError(e -> log.error("{}Processing webhook Id {} failed due to error {}", serverRequest.exchange().getLogPrefix(), serverRequest.pathVariable(webhookIdPathVar), e.getMessage(), e))).build().filter((request, next) -> {
            int value = (int) request.headers().contentLength().getAsLong();
            Long allowedPayloadLimit = DataSize.parse(allowedPayloadSize).toBytes();
            if (request.headers().contentLength().isPresent() && value > allowedPayloadLimit.intValue()) return Mono.error(new PayloadLargeException("cannot process page size of more than " + allowedPayloadSize));
    ...
    private Mono<Git> saveDatainMongo(Map<String, String> result, String[] params) {
        ...
        GitRequest request = GitRequest.builder().commitMessage(String.valueOf(result.get(COMMITMESSAGE))).commitTime(String.valueOf(result.get(COMMIT_TIME))).before(String.valueOf(result.get("before"))).after(String.valueOf(result.get("after"))).before(String.valueOf(result.get("before"))).compare(String.valueOf(result.get(COMPARE))).commitName(String.valueOf(result.get(COMMITBY_NAME))).commitAvatar(String.valueOf(result.get("commitBy.avatar"))).name(String.valueOf(result.get("repo.name"))).branch(String.valueOf(result.get("branch"))).url(String.valueOf(result.get("repo.url"))).description(String.valueOf(result.get("repo.description"))).added(result.get(CHANGES_ADDED).length() > 0 ? new ArrayList<>(Arrays.asList(result.get(CHANGES_ADDED).split(","))) : new ArrayList<>()).modified(result.get(CHANGES_MODIFIED).length() > 0 ? new ArrayList<>(Arrays.asList(result.get(CHANGES_MODIFIED).split(","))) : new ArrayList<>()).removed(result.get(CHANGES_REMOVED).length() > 0 ? new ArrayList<>(Arrays.asList(result.get(CHANGES_REMOVED).split(","))) : new ArrayList<>()).build();
        return gitRepository.save(Git.builder().itemCode(itemCode).accountId(params[1]).ownerId(params[2]).realmId(params[0]).event("push").gitId(String.join("-", "W", String.valueOf(random.nextLong()))).commitMessage(request.getCommitMessage()).commitTime(request.getCommitTime()).changes(new Changes(request.getAdded().toArray(new String[0]), request.getModified().toArray(new String[0]), request.getRemoved().toArray(new String[0]))).repo(new Repo(request.getName(), request.getUrl(), request.getDescription())).commitBy(new Commitby(request.getCommitName(), request.getCommitAvatar())).branch(request.getBranch()).after(request.getAfter()).before(request.getBefore()).compare(request.getCompare()).build());
    }

    @Autowired
    @java.lang.SuppressWarnings("all")
    public WebhooksConsumerFunctionConfiguration(final TransformerService transformerService, final MappingDataService mappingDataService, final StreamBridge streamBridge, final WebhookConfigRepository repository, final GitRepository gitRepository) {
    ...
    @Value("${digite.cloud.fabric.runtimeEnv:local}")
    @java.lang.SuppressWarnings("all")
    public void setNamespace(final String namespace) {
        this.namespace = namespace;
    }

    @Value("${digite.cloud.swiftalk.piglet.kafka.webhook-request-id-header:X-REQUEST-ID}")
    @java.lang.SuppressWarnings("all")
    public void setWebhookRequestIdHeader(final String webhookRequestIdHeader) {
        this.webhookRequestIdHeader = webhookRequestIdHeader;
    }

    @Value("${digite.cloud.swiftalk.piglet.webhook-id-path-var:webhookId}")
    @java.lang.SuppressWarnings("all")
